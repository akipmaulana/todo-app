  ...
  action$.pipe(
    ...
    mergeMap(action =>
      ApiClient.projects
        ...
        .catch(error => Observable.of(appAction.requestFailed(error)))
    )
  );
    ...
    mergeMap(action =>
      ApiClient.projects
        ...
        .flatMap(ajaxResponse =>
          Observable.concat(
            Observable.of(
              appAction.requestSuccess(ActionType.PROJECT_ADD_SUCCESS)
            ),
...
export const updateEpic = action$ =>
  action$.pipe(
    ...
    mergeMap(action =>
      ApiClient.projects
        .update(action.id, action.newName)
        .flatMap(ajaxResponse =>
          Observable.concat(
            Observable.of(
              appAction.requestSuccess(ActionType.PROJECT_UPDATE_SUCCESS)
            ),
            Observable.of(projectAction.fetchProjects())
          )
        )
    ...
    mergeMap(action =>
      ApiClient.projects
        ...
        .flatMap(ajaxResponse =>
          Observable.concat(
            Observable.of(
              appAction.requestSuccess(ActionType.PROJECT_DELETE_SUCCESS)
            ),
            Observable.of(projectAction.fetchProjects())
          )
        )
...
export const closeEpic = action$ =>
  action$.pipe(
    ...
    mergeMap(action =>
      ApiClient.projects
        .close(action.data.id, action.data.isClosed)
        .flatMap(ajaxResponse =>
          Observable.concat(
            Observable.of(
              projectAction.closeProjectDidSucceed(ajaxResponse.response)
            )
          )
        )
        .catch(error => Observable.of(
            appAction.requestFailed(ActionType.PROJECT_CLOSE_FAILED, error)
        ))
    )
  );
